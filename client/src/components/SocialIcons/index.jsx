...
const SocialIcons = ({className}) => {
  const links = [
    ...
    // {  url: 'https://dribbble.com', type: 'dribbble' },
  ]

  return (
    <div className={cx(className, styles.root)}>
      {links.map(({url, icon, type}, index) => (
        <a key={index} href={url} className={cx('icon', styles.icon, styles[type])} target="_blank" rel="noopener">
          <FontAwesomeIcon icon={!icon ? ['fab', type] : icon} />
        </a>
      ))}
      {/* dribbble wasn't in fortawesome library */}
      <a href="https://dribbble.com" className={cx('icon', styles.icon, styles.dribbble)}>
